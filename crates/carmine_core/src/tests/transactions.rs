...
fn test_multiple_collections_same_transaction() {
    ...
    if let CollectionHandle::StrStr(mut table) = coll1.write(&write_txn).unwrap() {
        table.insert("key1", "value1").unwrap();
    }

    if let CollectionHandle::IntInt(mut table) = coll2.write(&write_txn).unwrap() {
        table.insert(42, 100).unwrap();
    }

    if let CollectionHandle::NumberNumber(mut table) = coll3.write(&write_txn).unwrap() {
        table
    ...
    if let ReadOnlyCollectionHandle::StrStr(table) = coll1.read(&read_txn).unwrap() {
        assert_eq!(table.get("key1").unwrap().unwrap().value(), "value1");
    }

    if let ReadOnlyCollectionHandle::IntInt(table) = coll2.read(&read_txn).unwrap() {
        assert_eq!(table.get(42).unwrap().unwrap().value(), 100);
    }

    if let ReadOnlyCollectionHandle::NumberNumber(table) = coll3.read(&read_txn).unwrap() {
        assert_eq!(
...
fn test_read_write_isolation() {
    ...
    {
        let write_txn = store.handle.begin_write().unwrap();
        if let CollectionHandle::StrStr(mut table) = collection.write(&write_txn).unwrap() {
    ...
    if let ReadOnlyCollectionHandle::StrStr(table) = collection.read(&new_read_txn).unwrap() {
